...
// c->s  {"a":"bs","s":{"users":{"quill":null}}}

// Memory.prototype.bulkGetSnapshot
// Memory.prototype.getOps
// Memory.prototype._getOpLog

// c->s  {"a":"op","v":0,"create":{"type":"quill"},"c":"users","d":"quill"}

// Memory.prototype.getSnapshot
// Memory.prototype.getOps
// Memory.prototype._getOpLog
// Memory.prototype.writeOp
// Memory.prototype._getOpLog
// Memory.prototype.writeSnapshot
// clone


function LiveDbRethinkDB() {
  if (!(this instanceof LiveDbRethinkDB)) return new LiveDbRethinkDB(dbConfig);
...
module.exports = LiveDbRethinkDB;

LiveDbRethinkDB.prototype.close = function() {};
...
LiveDbRethinkDB.prototype.writeSnapshot = function(cName, docName, snapshot, callback) {
...
// Get Snapshot
    ...
    r.connect({
      ...
      if (err) {
        ...
        r.db('collections').table('collection').filter(cName).without('id').run(connection, function(err, cursor) {
          if (err) {
            console.log('error', err);
          } else {
            cursor.toArray(function (err, row) {
            // TODO will these ever have multiple items in the response?
            cNameResponse = row;
...
LiveDbRethinkDB.prototype.query = function(liveDb, index, query, options, callback) {
...
LiveDbRethinkDB.prototype.queryDoc = function(liveDb, index, cName, docName, query, callback) {
...
// Internal function.
LiveDbRethinkDB.prototype._getOpLog = function(cName, docName) {
